...
TEST(StringUtilTest, TruncateUTF8ToByteSize) {
  ...
  EXPECT_EQ(output, "");
  EXPECT_FALSE(Truncated("\xe1\x80\xbf", kuint64max, &output));
...
TEST(StringUtilTest, StringToInt) {
  ...
  for (size_t i = 0; i < ARRAYSIZE_UNSAFE(cases); ++i) {
    ...
    EXPECT_EQ(cases[i].output, StringToInt(WideToUTF16(wide_input)));
    EXPECT_EQ(cases[i].success, StringToInt(WideToUTF16(wide_input),
                                            &output));
    EXPECT_EQ(cases[i].output, output);
  }
  ...
  EXPECT_FALSE(StringToInt(WideToUTF16(wide_input), &output));
  EXPECT_EQ(6, output);
}
...
TEST(StringUtilTest, StringToInt64) {
  ...
  for (size_t i = 0; i < ARRAYSIZE_UNSAFE(cases); ++i) {
    ...
    std::wstring wide_input = ASCIIToWide(cases[i].input);
    EXPECT_EQ(cases[i].output, StringToInt64(WideToUTF16(wide_input)));
    EXPECT_EQ(cases[i].success, StringToInt64(WideToUTF16(wide_input),
                                              &output));
    EXPECT_EQ(cases[i].output, output);
  }
  ...
  std::wstring wide_input = ASCIIToWide(input_string);
  EXPECT_FALSE(StringToInt64(WideToUTF16(wide_input), &output));
  EXPECT_EQ(6, output);
}
...
TEST(StringUtilTest, HexStringToInt) {
  ...
  for (size_t i = 0; i < ARRAYSIZE_UNSAFE(cases); ++i) {
    ...
    std::wstring wide_input = ASCIIToWide(cases[i].input);
    EXPECT_EQ(cases[i].output, HexStringToInt(WideToUTF16(wide_input)));
    EXPECT_EQ(cases[i].success, HexStringToInt(WideToUTF16(wide_input),
                                               &output));
    EXPECT_EQ(cases[i].output, output);
  }
  // One additional test to verify that conversion of numbers in strings with
  ...
  std::wstring wide_input = ASCIIToWide(input_string);
  EXPECT_FALSE(HexStringToInt(WideToUTF16(wide_input), &output));
  EXPECT_EQ(0xc0ffee, output);
}
...
TEST(StringUtilTest, HexStringToBytes) {
  ...
  for (size_t i = 0; i < ARRAYSIZE_UNSAFE(cases); ++i) {
    ...
    EXPECT_EQ(cases[i].success,
              HexStringToBytes(WideToUTF16(wide_input), &output)) <<
        i << ": " << cases[i].input;
    for (size_t j = 0; j < cases[i].output_len; ++j)
      compare.push_back(static_cast<uint8>(cases[i].output[j]));
    ASSERT_EQ(output.size(), compare.size()) << i << ": " << cases[i].input;
...
TEST(StringUtilTest, StringToDouble) {
  ...
  for (size_t i = 0; i < ARRAYSIZE_UNSAFE(cases); ++i) {
    ...
    EXPECT_DOUBLE_EQ(cases[i].output,
                     StringToDouble(WideToUTF16(wide_input)));
    EXPECT_EQ(cases[i].success, StringToDouble(WideToUTF16(wide_input),
                                               &output));
    EXPECT_DOUBLE_EQ(cases[i].output, output);
  }
  ...
  std::wstring wide_input = ASCIIToWide(input_string);
  EXPECT_FALSE(StringToDouble(WideToUTF16(wide_input), &output));
  EXPECT_DOUBLE_EQ(3.14, output);
}
...
TEST(StringUtilTest, GrowBoundary) {
  ...
  char src[buf_len + 1];  // Need extra one for NULL-terminator. // NOLINT
...
// Test for JoinVector
TEST(StringUtilTest, JoinVector) {
  std::vector<int> in;
  EXPECT_EQ("", JoinVector(in, ','));

  in.push_back(1);
  EXPECT_EQ("1", JoinVector(in, ','));

  in.push_back(3);
  in.push_back(6);
  EXPECT_EQ("1,3,6", JoinVector(in, ','));

  EXPECT_EQ("1|3|6", JoinVector(in, '|'));
}

// Test for JoinVector
TEST(StringUtilTest, JoinKeys) {
  std::map<int, string> map;
  EXPECT_EQ("", JoinKeys(&map, ","));

  map[1] = "a";
  EXPECT_EQ("1", JoinKeys(&map, ","));
  map[1] = "b";
  EXPECT_EQ("1", JoinKeys(&map, ","));

  map[3] = "b";
  map[2] = "b";
  EXPECT_EQ("1, 2, 3", JoinKeys(&map, ", "));
  EXPECT_EQ("1|2|3", JoinKeys(&map, "|"));
}

// Test for JoinVector
TEST(StringUtilTest, JoinValues) {
  base::hash_map<int, string> map;
  EXPECT_EQ("", JoinValues(&map, ","));

  map[1] = "a";
  EXPECT_EQ("a", JoinValues(&map, ","));
  map[1] = "b";
  EXPECT_EQ("b", JoinValues(&map, ","));

  map[3] = "c";
  map[2] = "a";
  // The order may differ if the hash function changes.
  EXPECT_EQ("b, a, c", JoinValues(&map, ", "));
  EXPECT_EQ("b|a|c", JoinValues(&map, "|"));
}
...
TEST(StringUtilTest, StringToLowerASCII) {
  ...
  StringToLowerASCII(&str2);
  EXPECT_EQ(str2, "hello world");
  StringToUpperASCII(&str2);
  EXPECT_EQ(str2, "HELLO WORLD");
  EXPECT_EQ(StringToLowerASCII(str), "hello world");
  string str3;
  StringToLowerASCII(str, &str3);
...

  {
    ...
    StringToLowerASCII(&str4);
    EXPECT_EQ(str4, "hello world");
    EXPECT_EQ(str, "Hello WorlD");
  }
  ...
  {
    string str = "Hello WorlD";
    string str2 = StringToLowerASCII(str);
    EXPECT_EQ(str2, "hello world");
    EXPECT_EQ(str, "Hello WorlD");
  }

  {
    string str = "Hello WorlD";
    string str2;
    StringToLowerASCII(str, &str2);
    EXPECT_EQ(str2, "hello world");
    EXPECT_EQ(str, "Hello WorlD");
  }

  EXPECT_EQ(StringToUpperASCII(str), "HELLO WORLD");
  EXPECT_EQ(ToUpperASCII('a'), 'A');
  EXPECT_EQ(ToUpperASCII('A'), 'A');
  EXPECT_EQ(ToUpperASCII('\0'), '\0');
  EXPECT_EQ(ToUpperASCII('\xff'), '\xff');
  EXPECT_EQ(ToLowerASCII('A'), 'a');
  EXPECT_EQ(ToLowerASCII('a'), 'a');
  EXPECT_EQ(ToLowerASCII('\0'), '\0');
  EXPECT_EQ(ToUpperASCII('\xff'), '\xff');

  string str5 = "iPhone Hello World 大S 大s iPad";
  string str6;
  StringToLowerASCII(str5, &str6);
  EXPECT_EQ(str6, "iphone hello world 大s 大s ipad");

  string empty_str = "";
  EXPECT_EQ(StringToLowerASCII(empty_str), "");
}
